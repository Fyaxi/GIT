...
class CaixaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $column_valor = new TDataGridColumn('valor', 'Total Caixa', 'center');
        ...
        try
        {
            ...
            if($pega_caixa->id == $key AND $pega_caixa->aberto == 'Y'){
                ...
                if( $PuxaRelatorioGeralCaixa > 0.00)
                {
                    ...
                    {
                        throw new Exception('<span style="font-weight: bold;">HÁ UM FATURAMENTO EM ABERTO PARA O CAIXA!</span>');
                    }
    ...
    {//date('Y-m-d')
        try
        {
            ...
            if( $pega_caixa->usuario == TSession::getValue('login') )
            {
                ...
                elseif (!$pega_caixa->situacao == 'C')
                {
                    throw new Exception('<span style="font-weight: bold;">O CAIXA AINDA NÃO FOI CONFERIDO</span>');
                }
                else
                {
                    throw new Exception('<span style="font-weight: bold;">FATURAMENTO JÁ FOI GERADO PARA ESSE CAIXA</span>');
                }
            }
            else
            {
                new TMessage('danger', '<span style="font-weight: bold;">SOMENTE O USUÁRIO QUE ABRIU O CAIXA<BR>PODE REALIZAR O FECHAMENTO<span>');
            }    
    ...
    public function FecharCaixa($param)
    {
        try
        {
            ...
            $pega_caixa = Caixa::find($key);
            $pega_recebimento = CaixaMov::PegarRecebimento( $pega_caixa->id );

            $recebimento = ContaReceber::find($pega_recebimento);

            if($pega_caixa->situacao == 'P')
            {
                if( $recebimento->origem == 'C')
                {
                    if( !empty($recebimento->dt_pagamento) && $recebimento->ativo == 'Y')
                    {
                        $pega_recebimentoPG = CaixaMov::PegarRecebimentoValor( $pega_recebimento );
                        
                        //echo 'Valor do Recebimento: $'.$pega_recebimentoPG.' ';
                        //echo '</br>';
                        //echo 'Valor Do Caixa: $'.$pega_caixa->valor.' ';

                        if ($pega_recebimentoPG == $pega_caixa->valor )
                        {
                            $pega_caixa->fechamento = date('Y-m-d');
                            $pega_caixa->aberto = 'N';
                            $pega_caixa->store();
                        ...
                        throw new Exception('<span style="font-weight: bold;"><A HREF="?class=ReceberForm&method=onEdit&id='.$pega_recebimento.'&register_state=false&key='.$pega_recebimento.'">O CAIXA NÃO PODE SER FECHADO!<BR>FAÇA A BAIXA DO RECEBIMENTO PARA FECHA O CAIXA.<A></span>');
                    }
                }
                else
                {
                    new TMessage('warning', '<span style="font-weight: bold;">O FATURAMENTO VINCULADO NÃO É DE ORIGEM CAIXA<span>');
                }
            }
            else
            {
                new TMessage('warning', 'O CAIXA NÃO ESTÁ MODO DE PAGAMENTO PARA FECHAR');
            }
        }
        catch (Exception $e)
        {
