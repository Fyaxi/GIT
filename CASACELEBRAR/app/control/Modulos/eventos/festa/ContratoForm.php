...
class ContratoForm extends TWindow
{
    ...
    function __construct($param)
    {
        ...
        $this->form->setFormTitle('Agendamento Festa');
        ...
        //$servico_id = new TDBUniqueSearch('list_servico_id[]', TSession::getValue('unit_database'), 'Servico', 'id', 'nome', null, TCriteria::create( ['ativo' => 'Y', 'tipo_servico_id' => 1] ));
        ...
        try
        {
            ...
            if($param['agendado'] == 'Y')
            {
                if($param['ativo'] == 'Y')
                {
                    ...
                    if(!$evento_criado)
                    {
                        $tipo_contrato = new TipoContrato($master->tipo_contrato_id);

                        $evento = new Evento;
                        $evento->cor            = $tipo_contrato->cor;
                        $evento->titulo         = 'Festa '.$master->aniversariante.'';
