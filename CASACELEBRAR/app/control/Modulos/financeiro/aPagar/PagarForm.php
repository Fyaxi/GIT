...
class PagarForm extends TWindow
{
    ...
    function __construct($param)
    {
        ...
        $this->form->setFormTitle('Pagamento Dividido');
        ...
        $forma_recebimento->setEditable(FALSE);
        $pessoa_id->setMinLength(0);
        
        // add form fields to the form
        $this->form->addFields( [new TFormSeparator('<small>Informações Do Favorecido</small>')] );
        $row = $this->form->addFields( [ new TLabel('Pagamento N°') ], [ $id ] );
        $row->layout = ['col-sm-2 control-label', 'col-sm-2' ];
        
        $this->form->addFields( [ new TLabel('Pessoa') ], [ $pessoa_id ] );

        $this->form->addFields( [new TFormSeparator('<small>Informações Do Pagamento</small>')] );
        ...
        $this->form->addFields( [ new TLabel('Forma de Pagamento') ], [ $forma_recebimento ] );
        $this->form->addFields( [ new TLabel('Valor Total') ], [ $valor_total ], [ new TLabel('Desconto Concedido') ], [ $desconto ], [ new TLabel('Valor a Pagar') ], [ $valor ]  );
        ...
        $forma_recebimento = new TDBUniqueSearch('list_servico_id[]', TSession::getValue('unit_database'), 'FormaPagamento', 'id', 'nome', null, $criteria);
        ...
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                if( $object->forma_recebimento > 1 )
                {
                    ...
                    new TMessage('error', '<span style="color:black;font-weight: bold;">UTILIZE A TELA DE PAGAMENTO ÚNICO<br>PARA MOFICIAR ESSE LANÇAMENTO</span>', $pos_action);
                }
    ...
    public static function onSave($param)
    {
        try
        {
            ...
            if (empty($master->id))
            {
                $master->ativo = 'Y';
                $master->origem = 'A';
                $master->forma_recebimento = 1;
            }
            
            if(!$master->forma_recebimento == 1)
            {
                throw new Exception('<span style="color:black;font-weight: bold;">FORMA DE PAGAMENTO INCORRETA</span><br>PARA DIVIDIR LANÇAMENTOS');
            }

            if( !empty($master->desconto) )
            {
                $valor_desconto = (float) str_replace(['.',','], ['','.'], $master->desconto);
                $fatura_total   = (float) str_replace(['.',','], ['','.'], $master->valor_total);
            ...
            if( !empty($param['list_servico_id']) AND is_array($param['list_servico_id']) )
            {
                ...
                {
                    if (!empty($forma_recebimento) AND $master->forma_recebimento == 1)
                    {
                        ...
                        $detail->store(); // Store Contas Pagar PG
                    }
                    else
                    {
                        throw new Exception('<span style="color:red;font-weight: bold;">ERRO NA DIVISÃO DO PAGAMENTO</span><br><span style="color:black;font-weight: bold;">FORMA DE PAGAMENTO INCORRETA<BR>OU<BR>NÃO INSERIDA</span>');
                    }
                }
                ...
                if(!empty($param['list_servico_id'][0]))
                {
                    ...
                    if(!empty($master->dt_pagamento))
                    {
                        if($master->valor == $total_lancamentos_r)
                        {
                            $master->desconto = str_replace(['.',','], ['','.'], $master->desconto);
                            $master->valor_total = str_replace(['.',','], ['','.'], $master->valor_total);
                            $master->valor = str_replace(['.',','], ['','.'], $master->valor);
                            $master->store(); // Store Contas Pagar
                            $data = new stdClass;
                            $data->id = $master->id;
                            TForm::sendData('form_ContaPagarPg', $data);
                            $pos_action = new TAction(['ContaPagarList', 'onReload'], ['register_state' => 'true']);
