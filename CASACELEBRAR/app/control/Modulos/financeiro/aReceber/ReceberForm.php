...
class ReceberForm extends TWindow
{
    ...
    function __construct($param)
    {
        ...
        //$forma_recebimento = new TDBUniqueSearch('forma_recebimento', TSession::getValue('unit_database'), 'FormaRecebimento', 'id', 'nome');
        ...
        //$this->form->addFields( [ new TLabel('Forma de Entrada') ], [ $forma_recebimento ] );
        ...
        $forma_recebimento = new TDBUniqueSearch('list_servico_id[]', TSession::getValue('unit_database'), 'FormaRecebimento', 'id', 'nome', null, $criteria);
    ...
    function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                if( $object->forma_recebimento == 0 ) //['ReceberForm', 'onEdit'], ['id'=>'{id}', 'register_state' => 'false']
                {
                    //$pos_action = new TAction(['ReceberForm', 'onEdit'], ['id'=>'{id}', 'register_state' => 'true']);
                    $pos_action = new TAction(['ContaReceberList', 'onReload'], ['register_state' => 'true']);
                    new TMessage('error', '<span style="color:black;font-weight: bold;">FORMA DE RECEBIMENTO INCORRETA</span>', $pos_action);
                }

                TScript::create(' $("select[name=\'pessoa_id\'").prop("disabled", true); ');

                if( !$object->VendedorID == NULL )
                {
                    TScript::create(' $("select[name=\'forma_recebimento\'").prop("disabled", true); ');
                }
        ...
        try
        {
            ...
            if( !empty($param['list_servico_id']) AND is_array($param['list_servico_id']) )
            {
                ...
                foreach( $param['list_servico_id'] as $row => $forma_recebimento)
                {
                    ...
                    {
                            throw new Exception('<span style="color:black;font-weight: bold;">FORMA DE RECEBIMENTO INCORRETA<BR>OU<BR>NÃO INSERIDA</span>');
                    }
                }
                ...
                if(!empty($param['list_servico_id'][0]))
                {
                    ...
                    if(!empty($master->dt_pagamento))
                    {
                        if($master->valor == $total_lancamentos_r)
                        {
                            ...
                            $data = new stdClass;
                            $data->id = $master->id;
                            TForm::sendData('form_ContaReceberPg', $data);
                            $pos_action = new TAction(['ContaReceberList', 'onReload'], ['register_state' => 'true']);
